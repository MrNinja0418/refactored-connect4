...
class Game {
  constructor() {
    ...
    this.playerColorsForm = document.getElementById("player-colors-form");
  ...
  startGame(event) {
    ...
    this.currPlayer = this.player1; // Set current player to player 1
    this.makeBoard();
    this.makeHtmlBoard();
  ...
  placeInTable(y, x) {
    const piece = document.createElement("div");
    piece.classList.add("piece");
    piece.classList.add(`p${this.currPlayer === this.player1 ? "1" : "2"}`);
    piece.style.top = -50 * (y + 2);
  ...
  handleClick(evt) {
    if (!this.gameStarted || this.gameOver) return; // Don't allow moves if the game has not started or is over
    // get x from ID of clicked cell
    const x = +evt.target.id;
    // get next spot in column (if none, ignore click)
